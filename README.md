...
Sends discover (multicast)

Home Assistant registers device

Device sends initial state

Device sends periodic pong

Home Assistant marks device online/offline
...
broker-less operation

easy packet inspection and debugging

fast recovery after Home Assistant restarts

over protocol-level security.

Intended deployment model

ET-Bus is not designed to be exposed to untrusted networks or the internet.
...
This mirrors common industrial control and field-bus deployments.

Roadmap note

Protocol-level security (encryption and authentication) may be explored in the future, but does not exist today and should not be assumed.
